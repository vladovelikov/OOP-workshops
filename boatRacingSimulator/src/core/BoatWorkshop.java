...
public class BoatWorkshop implements BoatFactory {
    ...
    public Boat produce(String[] args) throws ArgumentException, NonExistantModelException {
        ...
        if (typeContainer.contains("Row")) {
            ...
            boat = new PowerBoat(args[1], Integer.parseInt(args[2]), this.engineRepository.getEntity(args[3]),
                    this.engineRepository.getEntity(args[4]));
        } else if (typeContainer.contains("Yacht")) {
            boat = new Yacht(args[1], Integer.parseInt(args[2]), this.engineRepository.getEntity(args[3]),
